  ...
  <div class="mx-auto max-w-4xl space-y-6">
    ...
    <UCard v-else>
      ...
      <div class="grid gap-4 sm:grid-cols-3">
        <div class="rounded-xl bg-gray-50 p-4"><span class="text-xs text-body-muted">Incluídas no mês</span><strong class="mt-1 block text-2xl text-body-strong">{{ entitlement.freeConsultations }}</strong></div>
        <div class="rounded-xl bg-gray-50 p-4"><span class="text-xs text-body-muted">Utilizadas</span><strong class="mt-1 block text-2xl text-body-strong">{{ entitlement.usedFreeThisMonth }}</strong></div>
        <div class="rounded-xl bg-blue-50 p-4"><span class="text-xs text-body-muted">Disponíveis</span><strong class="mt-1 block text-2xl text-[var(--portal-accent)]">{{ entitlement.remainingFree }}</strong></div>
      </div>
