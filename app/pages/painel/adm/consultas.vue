<template>
  <div>
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-800">Consultas Agendadas</h1>
    ...
    <div class="bg-white rounded-xl shadow">
      <UTable :data="consultations" :columns="columns" :loading="pending" class="w-full">
        ...
        <template #time-cell="{ row }">
          <span class="text-gray-800">{{ row.original.time.slice(0, 5) }}</span>
        </template>

        <template #tutor_name-cell="{ row }">
          <span class="text-gray-600">{{ row.original.tutor_name }}</span>
        </template>
        ...
        <UCard class="relative w-full max-w-lg shadow-2xl z-10 flex flex-col max-h-full">
          <template #header>
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-semibold text-gray-800">Atribuir Veterinário</h3>
          ...
          <div class="flex flex-col gap-4 overflow-y-auto">
            ...
            <div class="grid grid-cols-1 gap-3 max-h-64 overflow-y-auto pr-2">
              <div
                ...
                :class="selectedVetIdForModal === vet.id ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'border-gray-200 hover:border-primary/30 hover:bg-gray-50'"
                @click="selectedVetIdForModal = vet.id"
              >
                <div class="flex items-center justify-between">
                  <p class="font-medium text-gray-800">{{ vet.name }}</p>
